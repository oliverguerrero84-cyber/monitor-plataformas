...
def main():
    ...
    hora_mx = (ahora - datetime.timedelta(hours=6)).strftime("%H:%M")
    ...
    nuevo, caidas, recuperadas, lentas = {}, [], [], []
    ...
    for clave, ruta, nombre in PAGINAS:
        ...
        nuevo[clave] = {"ok": r["ok"], "motivo": r["motivo"], "segundos": r["segundos"],
        ...
        if r["ok"] and r["segundos"] and r["segundos"] > UMBRAL_LENTO:
    ...
    if partes:
        ...
        with open(HISTORIAL, "a", encoding="utf-8") as fh:
            fh.write(json.dumps({"cuando": sello, "avisado": enviado,
                                 "caidas": [n for n, _ in caidas],
                                 "recuperadas": [n for n, _ in recuperadas],
                                 "detalle": texto}, ensure_ascii=False) + "\n")
    else:
        print("\nsin cambios respecto a la revisión anterior — no se avisa")

    if lentas:
        print("\nlentas (no generan aviso, quedan en el historial):")
        for n, s in lentas:
            print(f"  {n}: {s}s")
        with open(HISTORIAL, "a", encoding="utf-8") as fh:
            fh.write(json.dumps({"cuando": sello, "tipo": "lentitud",
                                 "paginas": dict(lentas)}, ensure_ascii=False) + "\n")
